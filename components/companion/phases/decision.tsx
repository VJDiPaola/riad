...
import { computePlanCosts, formatCurrency } from "@/lib/companion/year-ahead-utils"
...
export function DecisionPhase() {
  ...
  const recCost = useMemo(
    () => computePlanCosts(recommended, activeEvents, severity),
    [recommended, activeEvents, severity],
  )
  const altCost = useMemo(
    () => computePlanCosts(alternative, activeEvents, severity),
    [alternative, activeEvents, severity],
  )

  // Pull priorities directly from what the employee said in the life interview.
  const priorityQuotes = journal
    .filter((e) => e.kind === "voice" || e.kind === "decision")
    .slice(-4)

  const regretFlags: RegretFlag[] = useMemo(() => {
    const flags: RegretFlag[] = []

    // If they mentioned therapy or mental health and the recommended plan has a high deductible.
    const mentionsTherapy = journal.some((j) =>
      /therapy|mental|counsel|anxiety|stress/i.test(j.content),
    )
    ...
    // If they mentioned baby or pregnancy.
    const mentionsBaby = journal.some((j) => /baby|pregnan|maternity|child/i.test(j.content))
    if (mentionsBaby) {
      flags.push({
        id: "baby-oop-max",
        severity: "info",
        title:
          language === "es"
            ? "Un bebé en marzo cambia el cálculo"
            : "A baby in March changes the math",
        body:
          language === "es"
        ...
        mitigation:
          language === "es"
            ? `El tope de ${recommended.name} es ${formatCurrency(recommended.outOfPocketMax)}.`
            : `${recommended.name}'s ceiling is ${formatCurrency(recommended.outOfPocketMax)}.`,
      })
    }

    // Cost regret: if alternative would have been meaningfully cheaper.
    ...
    if (diff > 600) {
      flags.push({
        ...
        body:
          language === "es"
            ? `Diferencia anual estimada: ${formatCurrency(diff)}.`
            : `Estimated annual difference: ${formatCurrency(diff)}.`,
        mitigation:
          language === "es"
    ...
    return flags
  }, [journal, recommended, alternative, recCost, altCost, language])
            ...
            {priorityQuotes.map((q) => (
              ...
              >
                ...
                <p className="mt-1 text-[11px] uppercase tracking-[0.15em] text-muted-foreground">
                  {new Date(q.createdAt).toLocaleDateString(language === "es" ? "es-ES" : "en-US", {
                    month: "short",
                    day: "numeric",
                    hour: "numeric",
                    minute: "2-digit",
            ...
            <dl className="mt-3 grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <dd className="font-mono text-base text-foreground">
                  {formatCurrency(recCost.totalAnnual)}
                </dd>
              </div>
              <div>
                <dt className="text-[11px] uppercase tracking-[0.15em] text-muted-foreground">
                ...
                <dd className="font-mono text-base text-foreground">
                  {formatCurrency(recommended.outOfPocketMax)}
                </dd>
              </div>
            </dl>
          </div>
          ...
          <div className="flex flex-col gap-3 md:w-72">
            ...
            >
              ...
              <p className="mt-1 font-mono text-xs">
                {formatCurrency(altCost.totalAnnual)} /
                {language === "es" ? " año" : " yr"}
              </p>
            </button>
            ...
            {regretFlags.map((f) => (
              <li
                ...
                className={`rounded-xl border p-4 ${
                  f.severity === "warn"
                    ? "border-[var(--color-saffron)]/50 bg-[var(--color-saffron)]/10"
